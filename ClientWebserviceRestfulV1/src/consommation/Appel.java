...
public class Appel {
	...
	public String putJson(String action, Object unObj)
	{
		...
		if(response.getStatus() >= 200 && response.getStatus() <= 299) {
			 return "Mise Ã  jour effectuÃ©e";
		 }
		 else {
			System.out.println("Echec mise à jour "+response.getStatus());
			return "Echec mise Ã  jour" + response.getStatus();
		 }
	}
	...
	public String deleteJson(String action)
	{
		...
		Response response = target.request(MediaType.APPLICATION_JSON)
		 ...
		 if(response.getStatus() >= 200 && response.getStatus() <= 299) {
			 ...
			 System.out.println("Echec mise à jour "+response.getStatus());
			 return "Echec mise à jour "+response.getStatus();
		 }
	}
